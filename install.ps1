# camclave install.ps1 — Windows installer
# - pip-installs runtime deps (opencv-python, Pillow)
# - links skill/ into ~/.claude/skills/camclave/ and ~/.codex/skills/camclave/
#   (symlink if Developer Mode is on; recursive copy otherwise)
# - writes a `camclave.cmd` shim into %USERPROFILE%\.camclave\bin
...
function Write-Step($msg) { Write-Host "[camclave] $msg" -ForegroundColor Cyan }
function Write-Warn($msg) { Write-Host "[camclave] $msg" -ForegroundColor Yellow }
...
$shim = Join-Path $binDir "camclave.cmd"
$pyExe = $python.Source
$entry = Join-Path $skillSrc "scripts\camclave.py"
@"
@echo off
"$pyExe" "$entry" %*
...
Write-Host ""
Write-Host "[camclave] installed. Try this in a fresh terminal:" -ForegroundColor Green
Write-Host "    camclave start" -ForegroundColor Green
Write-Host "Then ask Claude Code or Codex: 'use camclave to show me what I'm holding'." -ForegroundColor Green
