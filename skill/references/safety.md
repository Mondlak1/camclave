...
| Rail | Where enforced |
| --- | --- |
| Capture refuses without an active session | `lookhere.py: cmd_capture` checks `active_session()` |
...
| TTL hard cap of 60 min | `session.py: MAX_TTL_SECONDS`, clamped in `lookhere.py` |
...
| Captures land only in `~/.lookhere/captures/` | default in `handle_capture_request` |
